...
[Download Jon's OPAL element layout proposal](../../etc/Proposal-Jon-OpalElementLayout.pdf).
...
The remaining cross-cutting aims are:

- Support the three layout schemes in one unified mechanism, instead of splitting responsibility between user-interface objects, `OpalBeamline`, and `Ring`.
- Remove the visitor pattern from layout. This should also simplify the tracker inheritance tree.

These aims are still good design targets. The current branch `391-placement-of-sbend-and-rbend-analytic-fields-including-fringe-fields` moves in the direction of explicit placement records, ports, and a clearer separation between nominal body placement and field-local coordinates. It does not yet implement the full proposal.
...
Net assessment: the branch is best understood as a bridge stage, not as the implementation of the proposal. It introduces useful placement vocabulary and explicit pose records, but the architecture is still centered on visitor-based collection, `FieldList`, compatibility placement, and special-case bend logic. The next design step would be to turn the port/placement vocabulary into the actual layout API so that straight elements, analytic bends, curved multipoles, and nested lines can all participate through the same interface.
